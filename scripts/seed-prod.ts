...
async function main() {
  ...
  let factureCount = 0
  const clients = [
    { nom: 'Kofi MENSAH', email: 'kofi@gmail.com' },
    { nom: 'Aissatou DIALLO', email: null },
    { nom: 'Rodrigue HOUNSA', email: 'rodrigue@gmail.com' },
    { nom: 'Marie AGOSSOU', email: null },
  ]
  for (let mois = 5; mois >= 0; mois--) {
    for (let i = 0; i < rnd(8, 12); i++) {
      ...
      await prisma.facture.create({
        data: {
          operateurId: demo.id, numero: num,
          clientNom: client.nom, clientEmail: client.email ?? undefined,
  ...
  for (let i = 0; i < 12; i++) {
    ...
    await prisma.dossierCVFD.create({
      data: {
        typeDetection: 'Activité non déclarée', sujetNom: `Opérateur ${i+1}`,
        description: 'Activité numérique sans LAN valide détectée sur les réseaux sociaux.',
  ...
  console.log('Seed OK — demo@snaen.bj / Demo1234! — admin@cnaen.bj / Admin1234!')
}
